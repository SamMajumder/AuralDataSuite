...
for (dir_name in dirs_to_create) {
  dir_path <- here("Aural_Aridity", dir_name)
...
# Define the URL
url <- "https://www.ncei.noaa.gov/pub/data/nidis/indices/nclimgrid-monthly/spei-pearson/nclimgrid-spei-pearson-01.nc"


# Define the destination file path using here()
dest_file <- here("Aural_Aridity",
                  "Datasets", 
                  "nclimgrid-spei-pearson-01.nc")
...
spei_data <- brick(here("Aural_Aridity","Datasets",
                        "nclimgrid-spei-pearson-01.nc"))
...
spei_data_subset <- subset(spei_data, 
                           781:nlayers(spei_data))
...
num_years <- ceiling(nlayers(spei_data_subset) / 12)
...
yearly_avg_list <- map(1:num_years, function(i) {
  ...
  if (i == num_years) {
    end_layer <- nlayers(spei_data_subset)
  } else {
    end_layer <- start_layer + 11
  }
  
  yearly_layers <- spei_data_subset[[start_layer:end_layer]]
...
df <- data.frame(Year = 1960:2023, 
                 SPEI = unlist(yearly_avg_list))


### save this file as a csv for sonification ###

write.csv(df,"Aural_Aridity/Datasets/SPEI.csv",
          row.names = FALSE)
...
animated_plot <- df %>% 
  ...
  geom_line(aes(color = SPEI),size = 1) +
  geom_point(aes(color = SPEI),size = 2) +
  scale_color_viridis_c(direction = -1) +
  labs(title = 'Standardized Precipitation Evapotranspiration in the Contiguous US', 
       x = 'Year',
       y = 'SPEI') +
  theme_bw() +
...
anim_save("SPEI.gif",animated_plot)
...
output_folder <- here("Aural_Aridity","Figures")
...
create_plot_for_year <- function(year) {
  ...
  p <- ggplot(data_for_year, aes(x = Long, y = Lat, fill = SPEI)) +
    geom_tile() +
    scale_fill_viridis_c(direction = -1) +
    theme_minimal() +
    labs(title = paste("SPEI for", year),
         x = "Longitude",
         y = "Latitude",
         fill = "SPEI")
